...
// henry: modified for using App::CB_LOC::RUNTABLES
cbuffer KernelCB : register( b6 ) {
...
Buffer<uint> _F_IT            : register( t2 );
Buffer<int> _F_ITa            : register( t3 );
Buffer<int> _E_IT             : register( t4 );
Buffer<uint> _V_IT             : register( t5 );
...
void CatmarkComputeFace( uint3 ID : SV_DispatchThreadID )
{
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void CatmarkComputeQuadFace(uint3 ID : SV_DispatchThreadID)
{
	int i = int(ID.x) + indexStart;
	if (i >= indexEnd) return;
	int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void CatmarkComputeTriQuadFace(uint3 ID : SV_DispatchThreadID)
{
	int i = int(ID.x) + indexStart;
	if (i >= indexEnd) return;
	int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void CatmarkComputeEdge( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void BilinearComputeEdge( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void BilinearComputeVertex( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void CatmarkComputeVertexA( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    int vid = i + vertexOffset;
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void CatmarkComputeVertexB( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    int vid = i + vertexOffset;
...
// Vertex-vertices compute Kernels 'B' / k_Dart and k_Smooth rules
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void LoopComputeVertexB( uint3 ID : SV_DispatchThreadID )
{
...
[numthreads(WORK_GROUP_SIZE, 1, 1)]
void EditAdd( uint3 ID : SV_DispatchThreadID )
{
    int i = int(ID.x) + indexStart;
    if (i >= indexEnd) return;
    i += tableOffset;
