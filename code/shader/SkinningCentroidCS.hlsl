...
Buffer<float4> centroidTmpSRV : register ( t0 );
RWBuffer<float4> centroidUAV : register ( u0 );
...
float4 getIncrValue(int i) {
	float4 x = centroidTmpSRV[i];
	x += tmp[i/1024];
	return x;
}
...
void SkinningCS(	uint3 blockIdx : SV_GroupID, 
	...
	if (GI == 0)
		centroidUAV[0] = getIncrValue(int(N)-1);
