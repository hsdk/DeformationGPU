...
PSOutput_Color PS_VoxelizeSolidForward(PSInput_Voxelize input) {
	PSOutput_Color output;
...
PSOutput_Color PS_VoxelizeSolidBackward(PSInput_Voxelize input) {
	PSOutput_Color output;
...
////// ADAPTIVE RENDERER VOXELIZATION
//
//float4 PS_VoxelizeSubDSolidBackward(centroid DS_OUTPUT In) : SV_TARGET {
//	
//	//PSOutput_Color output;
//
//	//// determine first voxel
...
//
//
//float4 PS_VoxelizeSubDSolidForward(centroid DS_OUTPUT In) : SV_TARGET {
//	// determine first voxel
//	float3 gridPos = In.pos.xyz / In.pos.w;
//	gridPos.z *= g_gridSize.z;
//
...
//PS_OUT_SCULPT
//#endif
//	PSMain(centroid DS_OUTPUT In
//#ifdef WITH_DISPLACEMENT
//	,uint PatchID : SV_PrimitiveID
//#endif
//	) : SV_TARGET
//{
//#ifndef WITH_DISPLACEMENT
//	float3 N = normalize(In.normal);
//#endif	
//	     
//#ifdef WITH_TEXCOORDS	
//	#ifdef WITH_DISPLACEMENT 
//		float2 uv = float2(In.tangent.w, In.bitangent.w);
//	   
//		float disp, dispDu, dispDv;
//		disp = dispDu = dispDv = 0.0f;
//		#ifdef WITH_MIPMAPS
//			disp = ComputeDisplacementQuadraticMipDependent(PatchID, uv, In.tessVerts, dispDu, dispDv);
//		#else
//			disp = ComputeDisplacementQuadratic(PatchID, uv, dispDu, dispDv);
//		#endif
//	  
//		disp *= g_fDisplacementScalar;
//		dispDu *= g_fDisplacementScalar;
//		dispDv *= g_fDisplacementScalar;
//		float3 n = normalize(cross(In.tangent.xyz, In.bitangent.xyz)); 
//	#ifndef WITH_WEINGARTEN
//		float3 Tangent = In.tangent.xyz + n * dispDu; 
//		float3 BiTangent = In.bitangent.xyz + n * dispDv;
//	#else
//		float3 Tangent = In.tangent.xyz + n * dispDu + In.Nu * disp;
//		float3 BiTangent = In.bitangent.xyz + n * dispDv + In.Nv * disp;
//	#endif
//		float3 N = normalize(cross(Tangent,BiTangent));
//		N = mul( (float3x3)g_mWorld, N  );
//	#endif
//
//	#ifdef WITH_TRADITIONAL
//		float2 uv = In.tex; 
//		float3 tangent = normalize(In.tangent.xyz);
//		float3 bitangent = normalize(In.bitangent.xyz);
//		float3 tn = g_NormalMap.SampleLevel(g_LinearSampler, In.tex.xy,0).xyz * 2.0 - 1.0;
...
//	float3 color = g_ObjectColor.xyz;
//	
//	ambient = color * 0.1f;
//	if (N.z >= 0.0)	{
//		diffuse = color * 0.9 * N.z;
//		specular = float3(1.0, 1.0, 1.0) * 0.9 * pow(N.z, 64.0f);
//	} else {
...
//	}  
//#ifdef WITH_TRADITIONAL
//	//diffuse = specular = float3(In.tex.x, In.tex.y, 0.0f) * 0.5;
//	////diffuse = specular = float3(In.tex.x, In.tex.x, In.tex.x);
//	//diffuse = specular = tangent;
//	//ambient = 0.0f;
//#endif
//
//#ifdef WITH_DISPLACEMENT
//	//#ifdef WITH_MIPMAPS
//	//float4 tessVerts = In.tessVerts;
//	//float c = (1.0f - uv.y) * ((1.0f - uv.x)*tessVerts.x + uv.x*tessVerts.y) + uv.y * ((1.0f - uv.x)*tessVerts.w + uv.x*tessVerts.z);
//	//c /= 32.0f;
//	//diffuse = specular = float4(c,c,c,1.0f);
//	//#endif
//
//	//diffuse = specular = (disp + 1.0f) * 0.25f;
//	//diffuse = specular = disp;
//	//ambient = 0.0f;
////#ifdef WITH_WEINGARTEN
//	//diffuse = specular =  float3(abs(In.Nu.x), abs(In.Nu.y), abs(In.Nu.z))*10.0f;
////#endif
//	//uint3 info = g_TexPerPatchInfo[PatchID].rgb;
//	////uint2 offset = uint2(info.x >> 16, info.x & 0xffff) + g_Overlap;
//	//uint2 offset = uint2(info.x & 0xffff, info.x >> 16) + g_Overlap;
//	//diffuse.xy = specular.xy = offset.xy / 1000.0f;
//	////diffuse = specular = 0.0f;
//	////diffuse.xy = In.tex.xy;
//	////diffuse = specular = float3(dispDu, dispDv, 0.0f)*0.5f; 
//	  
//	 
//	//uint3 info = g_TexPerPatchInfo[PatchID].rgb;
//	//uint2 offset = uint2(info.x & 0xffff, info.x >> 16) + g_Overlap;
//	//uint2 localOffset = uint2(info.y & 0xffff, info.y >> 16);
//	//uint tileSize = 0x1 << (info.z & 0xffff);
//	//tileSize = tileSize >> g_Level;
//	//uint rots = info.z >> 16;
//	//
//	//if (rots == 1)	uv = float2(1.0f - uv.y, uv.x);
//	//if (rots == 2)	uv = float2(1.0f - uv.x, 1.0f - uv.y);
//	//if (rots == 3)	uv = float2(uv.y, 1.0f - uv.x);
//	////uv = 0.0f;
//// 
//	//ambient = 0.0f;
//	//diffuse = specular =  0.0f;
//	//diffuse.xy = specular.xy = (uv.xy * tileSize + localOffset.xy) / 32.0f;
//
//	//{
//	//	//outputs tex color - this one seems to work (above not)
//	//	uint3 info = g_TexPerPatchInfo[PatchID].rgb;
//	//	uint2 offset = uint2(info.x & 0xffff, info.x >> 16);
//	//	
//	//	uint numMips = (info.z & 0xffff);
//	//	uint2 localOffset = uint2(info.y & 0xffff, info.y >> 16);
//	//	float2 fLocalOffset = float2(localOffset) / (float)(0x1 << (16 - numMips));
//
//	//	uint uTileSize = 0x1 << numMips;
//	//	float tileSize = (float)uTileSize / (float)(0x1 << g_Level);
//	//	uint rots = info.z >> 16;
//
//	//	if (rots == 1)	uv = float2(1.0f - uv.y, uv.x);
//	//	if (rots == 2)	uv = float2(1.0f - uv.x, 1.0f - uv.y);
//	//	if (rots == 3)	uv = float2(uv.y, 1.0f - uv.x);
//
//	//	float2 coords = float2(uv.x*(float)(tileSize) + fLocalOffset.x, uv.y*(float)(tileSize) + fLocalOffset.y);
//	//	ambient = 0.0f;
//	//	diffuse = specular =  0.0f;
//	//	diffuse.xy = specular.xy = (coords) / 64.0f;
//	//}
//	
//
//#endif
//
//#ifndef WITH_SCULPT
//	return float4(ambient + diffuse + specular, 1.0);// * checker;
//#else
//	PS_OUT_SCULPT output;
...
//		uint2 offset = uint2(info.x & 0xffff, info.x >> 16);
//
//		output.offsets.x = offset.x >> 8;
//		output.offsets.y = offset.x & 0xff;
//		output.offsets.z = offset.y >> 8;
//		output.offsets.w = offset.y & 0xff;
//		
//		uint numMips = (info.z & 0xffff);
//		uint2 localOffset = uint2(info.y & 0xffff, info.y >> 16);
//		float2 fLocalOffset = float2(localOffset) / (float)(0x1 << (16 - numMips));
...
//		if (rots == 3)	uv = float2(uv.y, 1.0f - uv.x);
//
//		float2 coords = float2(uv.x*(float)(tileSize-1) + fLocalOffset.x, uv.y*(float)(tileSize-1) + fLocalOffset.y);
//		uint2 iuv = uint2(round(coords.x), round(coords.y));
//
//		output.texcoords.x = iuv.x >> 8;
//		output.texcoords.y = iuv.x & 0xff;
//		output.texcoords.z = iuv.y >> 8;
//		output.texcoords.w = iuv.y & 0xff;
//	#endif
//	return output;
//#endif
//} 
