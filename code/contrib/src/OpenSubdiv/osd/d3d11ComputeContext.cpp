...
namespace OpenSubdiv {
	namespace OPENSUBDIV_VERSION {
		...
		OsdD3D11ComputeHEditTable::OsdD3D11ComputeHEditTable(
			const FarVertexEditTables::VertexEditBatch &batch, ID3D11DeviceContext1 *deviceContext)
			...
			ID3D11DeviceContext1 *deviceContext)
		{
			...
			if (subdivisionTables->GetNumTables() > 5)
			{
				...
				_tables[FarSubdivisionTables::F_ITa] = subdivisionTables->Get_F_ITa().size() == 0 ? nullptr : new OsdD3D11ComputeTable(subdivisionTables->Get_F_ITa(), deviceContext, DXGI_FORMAT_R32_SINT);
			}
			else
			{
		...
		void
			OsdD3D11ComputeContext::UnbindEditShaderStorageBuffers(ID3D11DeviceContext1 *deviceContext) const
		{
		...
		void
			OsdD3D11ComputeContext::BindShaderStorageBuffers(ID3D11DeviceContext1 *deviceContext) const
		{
			...
			if (_tables[FarSubdivisionTables::F_IT]) {
				ID3D11ShaderResourceView *SRViews[] = {
					...
					_tables[FarSubdivisionTables::F_ITa] ? _tables[FarSubdivisionTables::F_ITa]->GetSRV() : NULL,
				};
		...
		void
			OsdD3D11ComputeContext::UnbindShaderStorageBuffers(ID3D11DeviceContext1 *deviceContext) const
		{
			ID3D11UnorderedAccessView *UAViews[] = { 0, 0 };
			deviceContext->CSSetUnorderedAccessViews(0, 2, UAViews, 0); // u0-u2
