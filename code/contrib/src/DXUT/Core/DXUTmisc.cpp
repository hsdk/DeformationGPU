...
typedef INT         (WINAPI * LPD3DPERF_BEGINEVENT)(DirectX::FXMVECTOR, LPCWSTR);
typedef INT         (WINAPI * LPD3DPERF_ENDEVENT)(void);
typedef VOID        (WINAPI * LPD3DPERF_SETMARKER)(DirectX::FXMVECTOR, LPCWSTR);
typedef VOID        (WINAPI * LPD3DPERF_SETREGION)(DirectX::FXMVECTOR, LPCWSTR);
...
bool DXUT_EnsureD3D11APIs( void )
{
    ...
    if( !s_DynamicCreateDXGIFactory )
    {
		...
		//s_hModDXGI = LoadLibrary( L"d3d11.dll" );
        if( s_hModDXGI )
        {
			//we will use CreateDXGIFactory1 to create a DXGIFactory2 on windows 8
...
HRESULT DXUTSnapD3D11Screenshot( LPCTSTR szFileName)//, D3DX11_IMAGE_FILE_FORMAT iff )
{
	...
	ID3D11DeviceContext *immContext  = DXUTGetD3D11DeviceContext();
	if (!immContext) {
	...
	if ( SUCCEEDED(hr) )
	{
		hr = DirectX::SaveWICTextureToFile( immContext, pBackBuffer,
			GUID_ContainerFormatPng, szFileName );
		pBackBuffer->Release();
	}
    
 //   ID3D11Texture2D* pBackBuffer;
 //   hr = pSwap->GetBuffer( 1, __uuidof( *pBackBuffer ), ( LPVOID* )&pBackBuffer );
 //   if (hr != S_OK)
 //       return hr;
 ...
 ////   }
	...
	////hr = DirectX::SaveDDSTextureToFile(dc,pCompatableTexture, szFileName);
