...
protected:
    struct STATE
    {
		...
		IDXGISwapChain1			 *m_DXGISwapChain;          // the D3D11 swapchain
		IDXGIFactory2			 *m_DXGIFactory;             // DXGI Factory object
		IDXGIAdapter1			 *m_DXGIAdapter;            // The DXGI adapter object for the D3D11 device		
		ID3D11Device1			 *m_D3D11Device;             // the D3D11 rendering device
		ID3D11DeviceContext1	 *m_D3D11DeviceContext;	   // the D3D11 immediate device context		
#else
		...
		ID3D11ShaderResourceView	*m_D3D11DepthStencilViewSRV;       // the D3D11 depth stencil texture (optional)
...
public:
    ...
    // D3D11 specific
#ifdef DIRECTX_11_1_ENABLED
    GET_SET_ACCESSOR( IDXGIFactory2*, DXGIFactory );
    GET_SET_ACCESSOR( IDXGIAdapter1*, DXGIAdapter );
	GET_SET_ACCESSOR( IDXGISwapChain1*, DXGISwapChain );
	GET_SET_ACCESSOR( ID3D11Device1*, D3D11Device );
	GET_SET_ACCESSOR( ID3D11DeviceContext1*, D3D11DeviceContext );
#else
	...
	GET_SET_ACCESSOR( ID3D11ShaderResourceView*, D3D11DepthStencilViewSRV );   
	...
	GET_SET_ACCESSOR( bool, OverrideMSAA );
...
//--------------------------------------------------------------------------------------
void DXUTParseCommandLine(_In_z_ WCHAR* strCommandLine,	_In_ bool bIgnoreFirstCommand = true);
...
//#ifdef DIRECTX_11_1_ENABLED
                          ID3D11Device1* pd3d11DeviceFromApp,
//#else
...
// Direct3D 11
HRESULT DXUTCreateD3D11Views(_In_ ID3D11Device1* pd3dDevice, _In_ ID3D11DeviceContext* pd3dDeviceContext, _In_ DXUTDeviceSettings* pDeviceSettings);
HRESULT DXUTCreate3DEnvironment11(_In_ ID3D11Device1* pd3dDeviceFromApp);
...
{ 
    ...
    //else
#ifdef DIRECTX_11_1_ENABLED
    return pNewDeviceSettings->d3d11.sd.Width; 
#else
...
{ 
    ...
    //else
#ifdef DIRECTX_11_1_ENABLED
        return pNewDeviceSettings->d3d11.sd.Height; 
#else
...
{ 
    ...
    //else
#ifdef DIRECTX_11_1_ENABLED
        return pNewDeviceSettings->d3d11.sdfs.Windowed ? true : false;
#else
...
ID3D11Device1* WINAPI DXUTGetD3D11Device()                  { return GetDXUTState().GetD3D11Device(); }
ID3D11DeviceContext1* WINAPI DXUTGetD3D11DeviceContext()    { return GetDXUTState().GetD3D11DeviceContext(); }
IDXGISwapChain1* WINAPI DXUTGetDXGISwapChain()              { return GetDXUTState().GetDXGISwapChain(); }
IDXGIFactory2* WINAPI DXUTGetDXGIFactory()                  { DXUTDelayLoadDXGI(); return GetDXUTState().GetDXGIFactory(); }
...
ID3D11ShaderResourceView* WINAPI DXUTGetD3D11DepthStencilViewSRV() {return GetDXUTState().GetD3D11DepthStencilViewSRV();}
...
bool bIgnoreFirstCommand)
{
	...
	for (int iArg = iArgStart; iArg < nNumArgs; iArg++)
	{
		...
		if (*strCmdLine == L'/' || *strCmdLine == L'-')
		{
			...
			if (DXUTIsNextArg(strCmdLine, L"msaa"))
			{
				GetDXUTState().SetOverrideMSAA(true);
				continue;
			}
		}
    ...
    switch( uMsg )
    {
        case WM_PAINT:
        {
                ...
                GetDXUTState().GetDeviceObjectsCreated() && GetDXUTState().GetDeviceObjectsReset() )
            {
               ...
               // {
#ifdef DIRECTX_11_1_ENABLED
					ID3D11Device1* pd3dDevice = DXUTGetD3D11Device();
					ID3D11DeviceContext1 *pDeferred = DXUTGetD3D11DeviceContext();
#else
...
//======================================================================================
_Use_decl_annotations_
HRESULT WINAPI DXUTCreateDevice(D3D_FEATURE_LEVEL reqFL,  bool bWindowed, int nSuggestedWidth, int nSuggestedHeight, int msaaCount, int msaaQuality ) {
    HRESULT hr = S_OK;
...
#ifdef DIRECTX_11_1_ENABLED
    deviceSettings.d3d11.sd.Width = nSuggestedWidth;
    deviceSettings.d3d11.sd.Height = nSuggestedHeight;
	deviceSettings.d3d11.sdfs.Windowed = bWindowed;
#else
	...
	deviceSettings.d3d11.sd.SampleDesc.Count   = msaaCount;
	deviceSettings.d3d11.sd.SampleDesc.Quality = msaaQuality; // coverageSamples
...
#ifdef DIRECTX_11_1_ENABLED
			deviceSettings.d3d11.sd.Width = 0;
			deviceSettings.d3d11.sd.Height = 0;
...
{
#ifdef DIRECTX_11_1_ENABLED
    IDXGIFactory2* pDXGIFactory = GetDXUTState().GetDXGIFactory();	
    if( pDXGIFactory == NULL )
    {
        DXUT_Dynamic_CreateDXGIFactory1( __uuidof( IDXGIFactory2 ), ( LPVOID* )&pDXGIFactory );
...
void DXUTUpdateDeviceSettingsWithOverrides(_Inout_ DXUTDeviceSettings* pDeviceSettings)
{
	...
	if( GetDXUTState().GetOverrideWidth() != 0 )
	{
		...
		pDeviceSettings->d3d11.sd.Width = GetDXUTState().GetOverrideWidth();
	}
	if( GetDXUTState().GetOverrideHeight() != 0 )
	{
		//pDeviceSettings->d3d9.pp.BackBufferHeight = GetDXUTState().GetOverrideHeight();
		pDeviceSettings->d3d11.sd.Height = GetDXUTState().GetOverrideHeight();
	}

	if( GetDXUTState().GetOverrideFullScreen() )
	{
		//pDeviceSettings->d3d9.pp.Windowed = FALSE;
		pDeviceSettings->d3d11.sdfs.Windowed = FALSE;
	}

	if( GetDXUTState().GetOverrideWindowed() ) {
		//pDeviceSettings->d3d9.pp.Windowed = TRUE;
		pDeviceSettings->d3d11.sdfs.Windowed = TRUE;
	}

	//if(GetDXUTState().GetOverrideMSAA())
	//{
	//	pDeviceSettings->d3d11.sd.SampleDesc.Count = 8;
	//	pDeviceSettings->d3d11.sd.SampleDesc.Quality = 31;
	//	exit(1);
	//}
#else
    // Override with settings from the command line
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTSetupD3D11Views( ID3D11DeviceContext1* pd3dDeviceContext )
{
	HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT DXUTCreateD3D11Views( ID3D11Device1* pd3dDevice, ID3D11DeviceContext1* pd3dImmediateContext,
							  DXUTDeviceSettings* pDeviceSettings )
{
	HRESULT hr = S_OK;
	IDXGISwapChain1* pSwapChain = DXUTGetDXGISwapChain();
	...
	V_RETURN(pd3dDevice->CreateRenderTargetView( pBackBuffer, NULL, &pRTV ));
	...
	if( pDeviceSettings->d3d11.AutoCreateDepthStencil )
	{
		...
		V_RETURN(pd3dDevice->CreateTexture2D( &descDepth, NULL, &pDepthStencilTex ));		
		GetDXUTState().SetD3D11DepthStencil( pDepthStencilTex );
		...
		if( pDeviceSettings->d3d11.AutoDepthStencilFormat == DXGI_FORMAT_R24G8_TYPELESS)
		{
			descDSV.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
			descSRV.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
		}
		else if(pDeviceSettings->d3d11.AutoDepthStencilFormat == DXGI_FORMAT_R32_TYPELESS)
		{
			descDSV.Format = DXGI_FORMAT_D32_FLOAT;
			descSRV.Format = DXGI_FORMAT_R32_FLOAT;			
		}
		else if(pDeviceSettings->d3d11.AutoDepthStencilFormat == DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS)
		{
		...
		V_RETURN(pd3dDevice->CreateDepthStencilView( pDepthStencilTex, &descDSV, &pDSV ));
		GetDXUTState().SetD3D11DepthStencilView( pDSV );

		descSRV.Texture2D.MipLevels = 1;
		descSRV.Texture2D.MostDetailedMip = 0;		

		ID3D11ShaderResourceView* pDepthStencilSRV=NULL;
		V_RETURN(pd3dDevice->CreateShaderResourceView(pDepthStencilTex, &descSRV, &pDepthStencilSRV));
		GetDXUTState().SetD3D11DepthStencilViewSRV(pDepthStencilSRV);		
	}
...
//--------------------------------------------------------------------------------------
HRESULT DXUTCreate3DEnvironment11(_In_ ID3D11Device1* pd3d11DeviceFromApp)
{
    HRESULT hr = S_OK;

	ID3D11Device1* pd3d11Device = nullptr;
	ID3D11DeviceContext1* pd3dImmediateContext = nullptr;
	...
	ID3D11Device* tmpDevice = NULL;                      // the device interface
	ID3D11DeviceContext* tmpDeviceContext = NULL;        // the device context interface

    IDXGIFactory2* pDXGIFactory = DXUTGetDXGIFactory();
	assert(pDXGIFactory);
	_Analysis_assume_(pDXGIFactory);

	// move this after swap chain creation
	...
	if (!pd3d11DeviceFromApp)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
#ifdef DIRECTX_11_1_ENABLED
				V_RETURN(tmpDevice->QueryInterface(__uuidof(ID3D11Device1), (LPVOID*)&pd3d11Device));
				V_RETURN(tmpDeviceContext->QueryInterface(__uuidof(ID3D11DeviceContext1), (LPVOID*)&pd3dImmediateContext));
				SAFE_RELEASE(tmpDevice);
				SAFE_RELEASE(tmpDeviceContext);
#else
				V_RETURN(tmpDevice->QueryInterface(__uuidof(ID3D11Device), (LPVOID*)&pd3d11Device));
				V_RETURN(tmpDeviceContext->QueryInterface(__uuidof(ID3D11DeviceContext), (LPVOID*)&pd3dImmediateContext));
				SAFE_RELEASE(tmpDevice);
				SAFE_RELEASE(tmpDeviceContext);
#endif
			}
...
#ifdef DIRECTX_11_1_ENABLED
            IDXGIDevice2* pDXGIDev = NULL;
            hr = pd3d11Device->QueryInterface( __uuidof( IDXGIDevice2 ), ( LPVOID* )&pDXGIDev );
		...
		// test
		// uncomment next 3 lines for flip mode
		//pNewDeviceSettings->d3d11.sd.Scaling = DXGI_SCALING_NONE;
		//pNewDeviceSettings->d3d11.sd.SwapEffect = DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL;
		//pNewDeviceSettings->d3d11.sd.Format = DXGI_FORMAT_R8G8B8A8_UNORM;

		//pNewDeviceSettings->d3d11.sd.Scaling = DXGI_SCALING_NONE;
		//pNewDeviceSettings->d3d11.sd.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;
		//pNewDeviceSettings->d3d11.sd.Format = DXGI_FORMAT_R8G8B8A8_UNORM;

		//pNewDeviceSettings->d3d11.sdfs.RefreshRate.Numerator = 1;
		//pNewDeviceSettings->d3d11.sdfs.RefreshRate.Denominator = 0;

        // Create the swapchain
#ifdef DIRECTX_11_1_ENABLED		
		hr = GetDXUTState().GetDXGIFactory()->CreateSwapChainForHwnd( pd3d11Device, DXUTGetHWND(), &pNewDeviceSettings->d3d11.sd,&pNewDeviceSettings->d3d11.sdfs, NULL, &pSwapChain);
#else
        
		hr = DXUTGetDXGIFactory()->CreateSwapChain( pd3d11Device, &pNewDeviceSettings->d3d11.sd, &pSwapChain );		
#endif
		DXUT_SetDebugName(pSwapChain, "SwapChain");
        if( FAILED( hr ) )
        {
...
HRESULT DXUTReset3DEnvironment11()
{
	DXUTReportLiveObjectsDXGI(L"start of DXUTReset3DEnvironment11");
    HRESULT hr;
	...
	DXGI_SWAP_CHAIN_FULLSCREEN_DESC FSSCDesc;
	pSwapChain->GetFullscreenDesc(&FSSCDesc);
	...
	if( pDeviceSettings->d3d11.sdfs.Windowed && FSSCDesc.Windowed )
	{
		RECT rcWnd;
		GetClientRect( DXUTGetHWND(), &rcWnd );
		pDeviceSettings->d3d11.sd.Width  = rcWnd.right - rcWnd.left;
		pDeviceSettings->d3d11.sd.Height = rcWnd.bottom - rcWnd.top;
	}

	// If the app wants to switch from windowed to fullscreen or vice versa,
	// call the swapchain's SetFullscreenState
	// mode.
	if( FSSCDesc.Windowed != pDeviceSettings->d3d11.sdfs.Windowed )
	{
		// Set the fullscreen state
		if( pDeviceSettings->d3d11.sdfs.Windowed )
		{			
		...
		else
		{
			...
			//pSwapChain->res
			//V_RETURN( pSwapChain->ResizeTarget( &pDeviceSettings->d3d11.sd ) );
			//V_RETURN( pSwapChain->ResizeTarget( &pDeviceSettings->d3d11.sd ) );
			//std::cerr << "checkme " <<__FILE__ << ": " << __LINE__ << std::endl;
	...
	{
		if( pDeviceSettings->d3d11.sd.Width == SCDesc.Width &&
			pDeviceSettings->d3d11.sd.Height == SCDesc.Height &&
			pDeviceSettings->d3d11.sd.Format != SCDesc.Format )
		{
			DXUTResizeDXGIBuffers( 0, 0, !pDeviceSettings->d3d11.sdfs.Windowed );
			bDeferredDXGIAction = true;
		}
		else if( pDeviceSettings->d3d11.sd.Width != SCDesc.Width ||
			pDeviceSettings->d3d11.sd.Height != SCDesc.Height )
		{
...
void WINAPI DXUTRender3DEnvironment()
{
    ...
    if ( GetDXUTState().GetSaveScreenShot() ) {
        DXUTSnapD3D11Screenshot( GetDXUTState().GetScreenShotName());
    }
	...
	//CHECKME DXGI_PRESENT_DO_NOT_WAIT
	hr = pSwapChain->Present1( SyncInterval, dwFlags| DXGI_PRESENT_DO_NOT_WAIT, &parameters );
	//hr = pSwapChain->Present1( SyncInterval, dwFlags, &parameters );

	// TODO CHECKME
	pd3dImmediateContext->DiscardView(DXUTGetD3D11RenderTargetView());
	pd3dImmediateContext->DiscardView(DXUTGetD3D11DepthStencilView());
...
void ClearD3D11DeviceContext( ID3D11DeviceContext1* pd3dDeviceContext )
{
...
void DXUTCleanup3DEnvironment( _In_ bool bReleaseSettings )
{
 	...
 	if( DXUTGetD3D11Device() != NULL )
	{
		...
		ClearD3D11DeviceContext( DXUTGetD3D11DeviceContext() );
		...
		ID3D11ShaderResourceView* pDSSRV = GetDXUTState().GetD3D11DepthStencilViewSRV();
		SAFE_RELEASE( pDSSRV );
		GetDXUTState().SetD3D11DepthStencilViewSRV( NULL );
...
#ifdef DIRECTX_11_1_ENABLED
		IDXGISwapChain1* pSwapChain = GetDXUTState().GetDXGISwapChain();
#else
		...
		//if(GetDXUTState().GetD3D11DeviceContext())
		//	GetDXUTState().GetD3D11DeviceContext()->Release();
		...
		{
			UINT references = DXUTGetD3D11Device()->Release();
			if( references > 0 )
			{
				DXUTReportLiveObjectsDXGI();
...
HRESULT WINAPI DXUTToggleFullScreen()
{
...
#ifdef DIRECTX_11_1_ENABLED
	deviceSettings.d3d11.sdfs.Windowed = !deviceSettings.d3d11.sdfs.Windowed; // datut
	if (!deviceSettings.d3d11.sdfs.Windowed) 
	{
		DXGI_MODE_DESC adapterDesktopDisplayMode =	{	800, 600, { 60, 1 }, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB };
	...
	else 
	{
		...
		deviceSettings.d3d11.sd.Height = r.bottom;
		deviceSettings.d3d11.sd.Width = r.right;
	}
#else
...
void DXUTCheckForDXGIFullScreenSwitch()
{
	...
	{
#ifdef DIRECTX_11_1_ENABLED
		pDeviceSettings->d3d11.sdfs.Windowed = SCDesc.Windowed;

		DXUTDeviceSettings deviceSettings = DXUTGetDeviceSettings();

		if (bIsWindowed)
		{
			GetDXUTState().SetWindowBackBufferWidthAtModeChange(deviceSettings.d3d11.sd.Width);
			GetDXUTState().SetWindowBackBufferHeightAtModeChange(deviceSettings.d3d11.sd.Height);
		}
		else
		{
			GetDXUTState().SetFullScreenBackBufferWidthAtModeChange(deviceSettings.d3d11.sd.Width);
			GetDXUTState().SetFullScreenBackBufferHeightAtModeChange(deviceSettings.d3d11.sd.Height);
		}
#else
...
void DXUTResizeDXGIBuffers( UINT Width, UINT Height, BOOL bFullScreen )
{
...
#ifdef DIRECTX_11_1_ENABLED
	pDevSettings->d3d11.sdfs.Windowed = !bFullScreen;
#else	
	//IDXGISwapChain* pSwapChain = DXUTGetDXGISwapChain();
	...
	ID3D11ShaderResourceView* pDSVSRV = GetDXUTState().GetD3D11DepthStencilViewSRV();
	SAFE_RELEASE( pDSVSRV );
	GetDXUTState().SetD3D11DepthStencilViewSRV(nullptr);
    ...
    // ResizeBuffers
	#ifdef DIRECTX_11_1_ENABLED
	//V( pSwapChain->ResizeBuffers( 2, 800, 600, DXGI_FORMAT_UNKNOWN, Flags	) );
	V(pSwapChain->ResizeBuffers( 0, 0, 0, DXGI_FORMAT_UNKNOWN, Flags	));
	//hr = pSwapChain->ResizeBuffers( 0, 0, 0, DXGI_FORMAT_R8G8B8A8_UNORM, Flags	);
	V(hr);
	//DXUTReportLiveObjectsDXGI();
	//V( pSwapChain->ResizeBuffers( pDevSettings->d3d11.sd.BufferCount,
	//	Width,
	//	Height,
	//	pDevSettings->d3d11.sd.Format,
	//	//DXGI_FORMAT_UNKNOWN,
	//	Flags		
	//	) );
	#else
    ...
    {
#ifdef DIRECTX_11_1_ENABLED
		pDevSettings->d3d11.sd.Width = ( UINT )rcCurrentClient.right;
		pDevSettings->d3d11.sd.Height = ( UINT )rcCurrentClient.bottom;
#else
	...
	// CHECKME
	// see AddRef on start of function
	//SAFE_RELEASE(pSwapChain);
}
...
void DXUTCheckForDXGIBufferChange()
{
    if(DXUTGetDXGISwapChain() != NULL && !GetDXUTState().GetReleasingSwapChain() )
    {
        // Determine if we're fullscreen
        BOOL bFullScreen;
        DXUTGetDXGISwapChain()->GetFullscreenState( &bFullScreen, NULL );
...
#ifdef DIRECTX_11_1_ENABLED
		pDeviceSettings->d3d11.sd.Format, pDeviceSettings->d3d11.sdfs.Windowed );
#else
...
#ifdef DIRECTX_11_1_ENABLED
				pDeviceSettings->d3d11.sd.Width, pDeviceSettings->d3d11.sd.Height,
#else
...
void DXUTUpdateD3D11DeviceStats(D3D_DRIVER_TYPE DeviceType, D3D_FEATURE_LEVEL featureLevel, DXGI_ADAPTER_DESC* pAdapterDesc)
{
	...
	if (DeviceType == D3D_DRIVER_TYPE_HARDWARE)
	{
		...
		assert(pd3dEnum);
		_Analysis_assume_(pd3dEnum);
		CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo = 
			pd3dEnum->GetDeviceSettingsCombo( pDeviceSettings->d3d11.AdapterOrdinal,											 
#ifdef DIRECTX_11_1_ENABLED
			pDeviceSettings->d3d11.sd.Format, pDeviceSettings->d3d11.sdfs.Windowed );
#else
            pDeviceSettings->d3d11.sd.BufferDesc.Format, pDeviceSettings->d3d11.sd.Windowed );
...
//#endif
	...
	DXUTReportLiveObjectsDXGI(L"DXUTShutdown Report Live Objects: \n");
...
void DXUTApplyDefaultDeviceSettings(DXUTDeviceSettings *modifySettings) {
    ...
    modifySettings->d3d11.AutoDepthStencilFormat = DXGI_FORMAT_R24G8_TYPELESS;//DXGI_FORMAT_D24_UNORM_S8_UINT;
#if defined(DEBUG) || defined(_DEBUG)
...
#ifdef DIRECTX_11_1_ENABLED
	modifySettings->d3d11.sd.Format = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
	//modifySettings->d3d11.sd.Format = DXGI_FORMAT_R8G8B8A8_UNORM;//_SRGB;
	modifySettings->d3d11.sd.Height = 480;
	modifySettings->d3d11.sd.Width = 640;
	// CHECKME
	modifySettings->d3d11.sd.Scaling = DXGI_SCALING_STRETCH;
	//modifySettings->d3d11.sd.AlphaMode = DXGI_ALPHA_MODE_UNSPECIFIED;
	modifySettings->d3d11.sd.AlphaMode = DXGI_ALPHA_MODE_IGNORE;	
	modifySettings->d3d11.sdfs.RefreshRate.Numerator = 60;
	modifySettings->d3d11.sdfs.RefreshRate.Denominator = 1;
	modifySettings->d3d11.sdfs.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	modifySettings->d3d11.sdfs.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	modifySettings->d3d11.sdfs.Windowed = 1;
#else
    ...
    modifySettings->d3d11.sd.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH ;    // HENRY was 0
...
HRESULT DXUTSnapDeviceSettingsToEnumDevice(DXUTDeviceSettings* pDeviceSettings, bool forceEnum, D3D_FEATURE_LEVEL forceFL)
{
	...
	{
#ifdef DIRECTX_11_1_ENABLED
        pDeviceSettings->d3d11.sdfs.Windowed = 1;
#else
	...
	pDeviceSettings->d3d11.Output = pDeviceSettingsCombo->Output;
...
#ifdef DIRECTX_11_1_ENABLED
		pDeviceSettings->d3d11.sdfs.Windowed = pDeviceSettingsCombo->Windowed;
		...
		if (pDeviceSettingsCombo->pOutputInfo != NULL) {
			...
			if (!pDeviceSettingsCombo->Windowed) {
				...
				pDeviceSettings->d3d11.sd.Width = bestDisplayMode.Width;
				pDeviceSettings->d3d11.sdfs.RefreshRate.Numerator = bestDisplayMode.RefreshRate.Numerator;
				pDeviceSettings->d3d11.sdfs.RefreshRate.Denominator = bestDisplayMode.RefreshRate.Denominator;
				pDeviceSettings->d3d11.sdfs.Scaling = bestDisplayMode.Scaling;
				pDeviceSettings->d3d11.sdfs.ScanlineOrdering = bestDisplayMode.ScanlineOrdering;
			}
		}
#else
		...
		if (pDeviceSettings->d3d11.sd.SampleDesc.Quality > pDeviceSettingsCombo->multiSampleCountList[bestMSAAIndex] - 1)
			pDeviceSettings->d3d11.sd.SampleDesc.Quality = pDeviceSettingsCombo->multiSampleCountList[bestMSAAIndex] - 1;
#ifdef DIRECTX_11_1_ENABLED
			pDeviceSettings->d3d11.sd.Format = pDeviceSettingsCombo->BackBufferFormat;
#else
...
void WINAPI DXUTReportLiveObjectsDXGI( LPCWSTR msg )
{	
	static HMODULE s_hModDXGIDbg = NULL;
	static IDXGIDebug* s_pDXGIDebug = NULL;
	if(s_hModDXGIDbg == NULL || s_pDXGIDebug == NULL)
	{
		s_hModDXGIDbg = LoadLibrary( L"dxgidebug.dll" );
		typedef HRESULT(__stdcall *fPtr)(const IID&, void**); 
		if(s_hModDXGIDbg)
		{		
			fPtr DXGIGetDebugInterface = (fPtr)GetProcAddress(s_hModDXGIDbg, "DXGIGetDebugInterface");		
			DXGIGetDebugInterface(__uuidof(IDXGIDebug), (void**)&s_pDXGIDebug);
		}
	}
	...
	if(s_pDXGIDebug)
	{
		...
		s_pDXGIDebug->ReportLiveObjects(DXGI_DEBUG_ALL, DXGI_DEBUG_RLO_DETAIL);
	}
