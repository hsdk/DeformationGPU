...
void* pIsD3D11DeviceAcceptableFuncUserContext)
{
	...
	HRESULT hr;
    IDXGIFactory2* pFactory = DXUTGetDXGIFactory();
	if( pFactory == NULL )
        return E_FAIL;
...
)
{
    ...
    //---------------------
#ifdef DIRECTX_11_1_ENABLED
    if( pDeviceSettingsCombo->Windowed == pOptimalDeviceSettings->sdfs.Windowed )
...
#ifdef DIRECTX_11_1_ENABLED
        if( displayMode.Width == pOptimalDeviceSettings->sd.Width &&
            displayMode.Height == pOptimalDeviceSettings->sd.Height )
            bResolutionFound = true;

        unsigned int current = 
            (UINT) abs ((int)displayMode.Width  - (int)pOptimalDeviceSettings->sd.Width) + 
            (UINT) abs ((int)displayMode.Height - (int)pOptimalDeviceSettings->sd.Height );
#else
    ...
    //---------------------
#ifdef DIRECTX_11_1_ENABLED
	if( pDeviceSettingsCombo->BackBufferFormat == pOptimalDeviceSettings->sd.Format )
	{
		fCurRanking += fBackBufferFormatWeight;
	}
	else
	{
		int nBitDepthDelta = abs( ( long )DXUTGetDXGIColorChannelBits( pDeviceSettingsCombo->BackBufferFormat ) - ( long )DXUTGetDXGIColorChannelBits(pOptimalDeviceSettings->sd.Format ) );
		float fScale = std::max( 0.9f - ( float )nBitDepthDelta * 0.2f, 0.0f );
    ...
    else
    {
		...
		float fScale = DirectX::XMMax(0.9f - (float)nBitDepthDelta * 0.2f, 0.0f);
        fCurRanking += fScale * fBackBufferFormatWeight;
    }
#endif
	...
	if (pDeviceSettingsCombo->pOutputInfo)
	{
		for (size_t idm = 0; idm < pDeviceSettingsCombo->pOutputInfo->displayModeList.size(); idm++)
		{
			...
			if( fabs( float( displayMode.RefreshRate.Numerator ) / displayMode.RefreshRate.Denominator -
				float( pOptimalDeviceSettings->sdfs.RefreshRate.Numerator ) /
				pOptimalDeviceSettings->sdfs.RefreshRate.Denominator ) < 0.1f )
#else
			if (fabs(float(displayMode.RefreshRate.Numerator) / displayMode.RefreshRate.Denominator -
