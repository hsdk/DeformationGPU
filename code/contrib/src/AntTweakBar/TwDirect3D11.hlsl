...
TextPSInput TextureVS(float4 pos : POSITION, float4 color : COLOR, uint id : SV_VertexID)
{
    TextPSInput ps; 
    ps.Pos = pos + g_Offset;
    ps.Color = color; 
	ps.Tex = float2(0,0); 
	if(id == 1)	ps.Tex = float2(1,0); 
	if(id == 2)	ps.Tex = float2(0,1); 
	if(id == 3)	ps.Tex = float2(1,1); 
    return ps; 
}

float4 TexturePS(TextPSInput input) : SV_TARGET
{ 
	return g_Font.Sample(g_FontSampler, input.Tex); 
