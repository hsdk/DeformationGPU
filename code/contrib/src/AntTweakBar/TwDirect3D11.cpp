...
#ifdef _WIN64
#   ifdef _DEBUG
#       include "TwDirect3D11_LineRectVS_D.h"
#       include "TwDirect3D11_LineRectCstColorVS_D.h"
#       include "TwDirect3D11_LineRectPS_D.h"
#       include "TwDirect3D11_TextVS_D.h"
#       include "TwDirect3D11_TextCstColorVS_D.h"
#       include "TwDirect3D11_TextPS_D.h"
#       include "TwDirect3D11_TextureVS_D.h"
#       include "TwDirect3D11_TexturePS_D.h"
#   else
...
#       include "TwDirect3D11_TextureVS.h"
#       include "TwDirect3D11_TexturePS.h"
#   endif
#else
#   ifdef _DEBUG
#       include "TwDirect3D11_LineRectVS_D.h"
...
int CTwGraphDirect3D11::Init()
{
	...
	hr = m_D3DDev->CreateVertexShader(g_TextureVS, sizeof(g_TextureVS), NULL, &m_TextureVS);
	if( FAILED(hr) )
	{
		g_TwMgr->SetLastError(g_ErrCreateVS11);
		Shut();
	...
	hr = m_D3DDev->CreatePixelShader(g_TexturePS, sizeof(g_TexturePS), NULL, &m_TexturePS);
	if( FAILED(hr) )
	{
		g_TwMgr->SetLastError(g_ErrCreatePS11);
		Shut();
...
void CTwGraphDirect3D11::DrawRectTex( int _X0, int _Y0, int _X1, int _Y1, void* texID )
{
	assert(m_Drawing==true);	
	// border adjustment
	if(_X0<_X1)
	...
	if( SUCCEEDED(hr) )
	{
		...
		ID3D11ShaderResourceView* tex = (ID3D11ShaderResourceView*)texID;
		//ID3D11ShaderResourceView* texPP[] = {(ID3D11ShaderResourceView*)texID};		
		m_D3DDevImmContext->PSSetShaderResources(0, 1, &tex);
		m_D3DDevImmContext->PSSetSamplers(0, 1, &m_SamplerState);
		m_D3DDevImmContext->VSSetConstantBuffers(0, 1, &m_ConstantBuffer);
		m_D3DDevImmContext->VSSetShader(m_TextureVS, NULL, 0);
		m_D3DDevImmContext->PSSetShader(m_TexturePS, NULL, 0);
		m_D3DDevImmContext->Draw(4, 0);

		ID3D11ShaderResourceView* srvPPNULL[] = {NULL};
		ID3D11SamplerState* ssPPNULL[] = {NULL};		
		m_D3DDevImmContext->PSSetShaderResources(0, 1, srvPPNULL);
		m_D3DDevImmContext->PSSetSamplers(0, 1, ssPPNULL);
	}
}
//  ---------------------------------------------------------------------------
